...
export const ValidateDtoUpdateService = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  if (service.rangeOfHoursToWork) {
    const format: string[] | any = service.rangeOfHoursToWork.split(',');
    ...
    if (format !== 2) {
      ReponseHttp.BAD_REQUEST(
        res,
        'THE rangeOfHoursToWork Have incorrect format',
        req.method,
      );
    }
  }
