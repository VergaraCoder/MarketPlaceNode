...
export const ValidateDtoUpdateProducts= (req: Request, res: Response, next: NextFunction)=>{
    ...
    if(product.description){
        ...
        if(product.description.length<10){
            ReponseHttp.BAD_REQUEST(res,"THE DESCRIPTION MUST HAVE MORE THAN 10 CHARACTERS",req.method);
            return;
        }
    }
    ...
    if(product.name){
        ...
        if(product.name.length<3){
            ReponseHttp.BAD_REQUEST(res,"THE NAME MUST HAVE MORE THAN 3 CHARACTERS",req.method);
            return;    
        }
    ...
    if(product.stock){
        ...
        if(product.stock==0){
            ReponseHttp.BAD_REQUEST(res,"THE STOCK AVAILABLE MUST BE MORE THAN 0",req.method);
            return;
        }
    }
    next();
