...
export class SeederPriceMode implements Seeder{
    async run(dataSource:DataSource): Promise<void> {
        const repoPriceMode:Repository<PriceMode>=dataSource.getRepository("priceMode");
        const dataPriceMode:DataPriceMode[]=[
            {name:"hora"},
            {name:"dia"},
        ];

        for(const x of dataPriceMode){
            const query:PriceMode | null =await repoPriceMode.findOneBy({name:x.name});
            if(!query){
